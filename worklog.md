...
sitemap/route-set equality, robots, feed completeness, and the
absence of localhost or /blog/undefined references.
...
A page's alternates object replaces the layout's: RSS
autodiscovery types must be repeated on the page that needs them.
...
- Sitemap + robots + feed all generated by the blog pipeline from
...
- RSS autodiscovery <link rel=alternate> now actually emitted on
  /blog/ (page-level alternates.types; the layout copy was dead
  config under shallow merging)
