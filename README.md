...
- **Unified route transition (v4.0.4)** — real route/tab navigation (HOME, ABOUT, BLOG, CONTACT, WORK, RESEARCH, topic hubs) shares one loading experience mounted once from the root layout (`GlobalRouteTransition`): intent captured on click, a 180 ms grace period so warmed/cached navigations never flash a loader, then the same 13-point-star signal language as the scene loader (variant `route` of `SceneLoadingScreen`) until the destination route actually commits. The surface is display-only — pointer-events none in every state, no fake percentage, no minimum duration, no navigation delay — race-safe under rapid clicks (monotonic intent tokens, latest navigation wins), excluded for external links, downloads, mailto/tel, modifier/new-tab clicks and same-route hash-scene navigation, inert in the static export, and static under reduced motion. The pure decision layer (`lib/routeIntent.ts`) and the timing constants (`lib/loadPhase.ts`) are unit-tested
- **The document-tab hero signal (v4.0.4)** — every ContentShell document (About, Contact, Work, Research, the topic hubs) carries one shared top-left identity animation beside the title block: a node inside a thin ring with the scene loader's transmission arc, absolutely positioned on the shared alignment rail (zero layout impact, never behind the text), themed per tab through the page accent, compositor-only (`transform`/`opacity`), and static under reduced motion
