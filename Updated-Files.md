...
## Release: v4.0.0 — MEDIA FOUNDATION + EMBEDDED MUSIC PLAYER (2026-09-19)

Mission: rename the user-facing Library to **Media** everywhere, replace the
old Library implementation with the new Media implementation, and add a real
embedded **Music player** — while preserving the existing architecture, the
RED MAGIC Runtime, the static export, SEO behavior, and the external Contents
contract.

### Media migration (Library → Media)

- `components/scenes/LibraryScene.tsx` → `components/scenes/MediaScene.tsx`
  (REPLACED, old file deleted): MEDIA kicker, filter group
  ALL · MUSIC · BOOKS · VIDEO · ART (order per the Media model), music track
  rows with inline play, per-track detail panel with PLAY / PLAY NEXT /
  ADD TO QUEUE, honest MUSIC empty state when no Music source publishes
  tracks, and the viewer modal kept for PDFs / video / images only (music
  plays through the global player — no per-item `<audio>` anymore).
- `components/scenes/LibraryScene.module.css` → `MediaScene.module.css`
  (class prefixes renamed `library*` → `media*`; new music empty-state,
  track-row, queue-action and track-meta styles; reduced-motion guarded).
- `components/LibraryPdfReader.tsx` → `components/MediaPdfReader.tsx`
  (+ module CSS): same HEAD + range-probe contract, renamed.
- `lib/contentRepository.ts` → `lib/mediaRepository.ts` (REPLACED, old file
  deleted): module-scope normalization of `data/media.json` into the
  discriminated `MediaItem` union; exports `listMedia`, `listMusic`,
  `getMediaItems`, Media vocabulary helpers.
- `lib/media/normalize.ts` (NEW): pure, import-free model core —
  `MediaItem = BookItem | MusicItem | VideoItem | ArtItem`, kind/category
  derivation, legacy `audio` → `music` alias, URL builders, Media filter
  vocabulary. Shared by the app and `node --test` (single source of truth).
- `data/library.json` → `data/media.json` (REPLACED): version 3 manifest;
  committed snapshot contains the 3 published books and ZERO music tracks
  (the documented empty-Music state). The live sync reproduces it
  byte-identically from the external contract.
- Scene id `library` → `media` across: `components/LivingShell.tsx`
  (SceneId, SCENES labels, alias map import), `components/SceneUrlSync.tsx`
  (`#library` → `#media` alias resolution + canonical normalization),
  `components/SceneRegistry.tsx`, `components/ScenePreloader.tsx`,
  `lib/navigation.ts` (WORLD_NAV MEDIA entry), `lib/worldSignals.ts`
  (WorldSceneMood), `data/routes.json` (scenes.names).
- `lib/sceneIds.ts` (NEW): neutral home of the SceneId type +
  SCENE_ID_ALIASES (`library → media`) so no component-to-component import
  cycle is created.
- CSS scene selectors renamed: `WorldBackground.module.css`
  (`[data-scene="library"]` → `"media"`), `UnifiedSiteNav.module.css`
  (`[data-id]`), `CompactMenu.module.css` (`[data-scene]`).
- User-facing copy updated in `components/scenes/WorkScene.tsx`,
  `lib/workRegistry.ts`, `lib/hubs.ts` (Contents / RED MAGIC books entries
  now say "Media collection", link `/#media`); blog sources
  `content/blog/the-anatomy-of-a-fast-static-site.md` and
  `content/blog/building-under-constraints.md` updated (`/#media`, "media
  record"); `data/blog/posts.json` regenerated by the pipeline.
- Comments/labels updated in `SceneNavigator`, `BlogHeader`, `SiteDocNav`,
  `UnifiedSiteNav`, `RedCursor`, `app/layout.tsx`, `lib/resourceStore.ts`,
  `eslint.config.mjs`. Brand "glyph library" strings are unrelated and were
  intentionally kept.

### Music data pipeline (build-time, zero dependencies)

- `scripts/media/bytes.mjs` (NEW): byte primitives, ID3 text decodings
  (ISO-8859-1/UTF-16/UTF-16BE/UTF-8), ID3v1 genre table, slash-pair and
  duration formatting.
- `scripts/media/id3.mjs` (NEW): ID3v2.2/2.3/2.4 tag parser — title, artist,
  album, album artist, year/date, genre, track, disc, composer, comment;
  whole-tag + per-frame unsynchronisation; extended headers; incomplete-tag
  `requiresBytes` contract for range windows.
- `scripts/media/mpeg.mjs` (NEW): MPEG frame header parser, first-frame
  location, Xing/Info frame-count duration, CBR fallback.
- `scripts/media/mp4.mjs` (NEW): ISO-BMFF atom walker (32/64-bit sizes),
  `mvhd` duration (v0 + v1), iTunes `ilst` tag decoding (`©nam ©ART ©alb
  aART ©day ©gen gnre trkn disk ©wrt ©cmt`).
- `scripts/media/flac.mjs` (NEW): STREAMINFO exact duration, VORBIS_COMMENT
  tag mapping; picture blocks intentionally skipped (covers are file-path
  resolved, never embedded-binary).
- `scripts/media/covers.mjs` (NEW): deterministic cover resolution —
  explicit path → matching audio basename → folder cover
  (cover/folder/front × jpeg/png) → fallback; no fuzzy matching.
- `scripts/media/audioMetadata.mjs` (NEW): range-request orchestration
  (probe → head window → tail window for end-moov M4A; single full read
  only for ≤1 MB files).
- `scripts/media/manifestSync.mjs` (NEW): pure sync core — external library
  manifest normalization, optional music manifest merge (absence = empty
  Music state, never fabricated tracks), embedded-metadata enrichment with
  manifest fallback, deterministic cover probes.
- `scripts/sync-media-manifest.mjs` (NEW): CLI used by CI ("Sync Media
  manifest"); fetches the external `Parsaetak/Contents/Projects/library.json`
  (CONTRACT INTACT — external file name unchanged), merges the optional
  `Projects/music.json`, enriches, and writes `data/media.json`.
- `scripts/media/manifestSchema.mjs` + `scripts/validate-media-manifest.mjs`
  (NEW, replaces `validate-library-manifest.mjs` which is DELETED): version-3
  schema gate — canonical types, type/extension coherence, music field
  typing, legacy `audio` rejection.
- Verified: `npm run media:sync` against the real external contract
  reproduces the committed `data/media.json` byte-identically, and reports
  the documented empty-Music state.

### Music player (one store, one audio element, lazy UI)

- `lib/player/playerStore.ts` (NEW): framework-free singleton store —
  injectable audio factory (tests use a scripted fake), ONE `<audio>` element
  created only on explicit play intent, real queue semantics (current /
  upcoming / history / play now / play next / add / remove / clear /
  select queued / clear upcoming), previous = restart when progressed
  (>3 s) else history step, end-of-track auto-advance with repeat
  off/all/one, honest stop when nothing remains, volume/mute persistence,
  error surfacing, Media Session integration, and the one-time
  `web:player:engage` DOM event that mounts the UI.
- `lib/player/usePlayer.ts`, `lib/player/format.ts` (NEW): React
  `useSyncExternalStore` binding + honest time formatting.
- `components/player/PlayerRoot.tsx` (NEW): mounted in `LivingShell`
  (survives scene + route navigation); imports NO player code — only a DOM
  event listener that triggers a dynamic import of the surface, so the
  entire player subsystem stays out of the initial bundle.
- `components/player/PlayerSurface.tsx`, `MiniPlayer.tsx`,
  `ExpandedPlayer.tsx`, `PlayerArtwork.tsx`, `Player.module.css` (NEW):
  desktop fixed bottom mini bar (artwork, identity, transport, seek slider
  with `aria-valuetext`, times, volume, mute, expand), mobile sticky
  compact bar (44 px targets, safe-area inset, tap-to-expand), expanded
  portal dialog (large artwork, metadata, full controls, repeat, queue with
  per-row play/remove + clear, history note), black/red laboratory styling,
  visible focus states, native range sliders, `prefers-reduced-motion`
  guards, artwork lazy-loading with a glyph fallback (never a placeholder
  request).

### Data / CI / SEO

- `.github/workflows/deploy.yml`: "Sync Library manifest" → "Sync Media
  manifest" (runs `scripts/sync-media-manifest.mjs`; the raw external-cURL
  step is gone while the external URL contract is byte-identical), 
  "Validate Library manifest" → "Validate Media manifest", plus a new
  "Run unit tests" gate before the build.
- `scripts/build-blog.mjs`: KNOWN_SCENES now `media` (canonical) with
  `library` kept as the documented legacy alias for article links.
- `scripts/verify-seo.mjs`: SCENE_HASHES derives from the registry
  (`media`) plus the legacy `library` alias, so pre-migration links keep
  verifying; comments updated.
- `package.json`: version 4.0.0; new scripts `media:sync`, `media:validate`,
  `test` (zero-dependency `node --test` on TypeScript via a resolve hook).
- `tests/` (NEW): 93 tests — ID3v2.3/v2.4 metadata, MP3 duration (Xing +
  CBR), M4A atoms + mvhd duration, FLAC STREAMINFO + Vorbis comments,
  range-pipeline extraction (in-memory + REAL local HTTP server with byte
  ranges), deterministic cover resolution, manifest schema gate, sync
  normalization (legacy alias, malformed records, no-fabrication laws),
  Media model/filter/labels, player store (queue, next/previous, repeat,
  auto-advance, errors, volume, single-element guarantee), a11y markup
  contract, and migration invariants (media.json v3, library.json gone,
  #media navigation, no #library article links, no fabricated audio in
  public/). Fixtures (`tests/fixtures/audio.mjs`) are synthetic test bytes
  only — never site content.

### Verification results (v4.0.0, recorded)

- `npm ci` — OK
- `npm test` — 93/93 pass
- `npm run media:sync` — external contract fetched, empty-Music state
  reported, `data/media.json` byte-identical to the committed snapshot
- `npm run blog` — OK (9 articles, sitemap 21 URLs)
- `npm run build` — OK (21 routes exported)
- `npm run lint` — 0 errors (22 warnings: the pre-existing
  `@next/next/no-img-element` class of warnings)
- `npx tsc --noEmit` — clean
- `npm run verify` (SEO + brand + export) — all passed
- Browser checks (desktop + mobile emulation): Media scene loads, MUSIC
  filter renders the honest empty state, books open the PDF reader,
  navigation `#media` canonical + `#library` alias normalize correctly,
  no audio/network requests before playback intent, RED MAGIC still
  lazy-loads. (Playback with a real track verified locally against a
  synthetic fixture through the same pipeline; no Music source exists in
  the Contents repository yet.)
